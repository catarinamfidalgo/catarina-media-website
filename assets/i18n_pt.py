# -*- coding: utf-8 -*-
"""European Portuguese strings. Keys are the exact English source text."""

# Interface chrome
UI = {
    "Video post-production": "Pós-produção de vídeo",
    "Home": "Início",
    "Portfolio": "Portfólio",
    "About": "Sobre",
    "For Agencies": "Para Agências",
    "Blog": "Blog",
    "Contact": "Contacto",
    "Start a project": "Começar um projeto",
    "View portfolio": "Ver portfólio",
    "Scroll": "Descer",
    # "Made in the edit" is deliberately left in English in every
    # language — it is a tagline, not a sentence, and the play on
    # "made in ..." does not survive translation.
    "Services": "Serviços",
    "Let's work <i>together.</i>": "Vamos <i>trabalhar juntos.</i>",
    "A white-label partner for agencies who need to <i>scale</i>":
        "Um parceiro white-label para agências que precisam de <i>escalar</i>",
    "Send message": "Enviar mensagem",
    "All rights reserved": "Todos os direitos reservados",
    "Video coming soon": "Vídeo em breve",
    "Tasks": "Tarefas",
    "Industry": "Setor",
    "Select a type…": "Selecione um tipo…",
    "Name": "Nome",
    "Email": "Email",
    "Company": "Empresa",
    "Type of video": "Tipo de vídeo",
    "Message": "Mensagem",
    "Other": "Outro",
    "projects": "projetos",
    "project": "projeto",
    "View project →": "Ver projeto →",
    "Videos": "Vídeos",
}

# Service list
SERVICES = {
    "Video Editing": "Edição de Vídeo",
    "I build the story out of raw footage and find the pace that holds it together.":
        "Construo a história a partir do material bruto e encontro o ritmo que a sustenta.",
    "Color Grading": "Correção de Cor",
    "Grading every shot so the piece feels like one film and fits your brand.":
        "Trato cada plano para que a peça pareça um só filme e encaixe na sua marca.",
    "Motion Graphics &amp; Animation": "Motion Graphics e Animação",
    "Titles, lower-thirds, logo animation, and light 2D work when a scene needs it.":
        "Títulos, oráculos, animação de logótipo e trabalho 2D leve quando a cena pede.",
    "Sound Design &amp; Mixing": "Sound Design e Mistura",
    "Cleaning up dialogue, balancing the music, and mixing for broadcast and streaming.":
        "Limpar diálogos, equilibrar a música e misturar para televisão e streaming.",
    "Subtitling &amp; Localization": "Legendagem e Localização",
    "Subtitles and full language versions in English, Portuguese, and Spanish.":
        "Legendas e versões completas em inglês, português e espanhol.",
    "Licensed Music &amp; Stock": "Música e Imagens Licenciadas",
    "Finding and licensing the right music and stock, cleared and ready to publish.":
        "Encontrar e licenciar a música e as imagens certas, já libertadas para publicação.",
}

# Body copy
COPY = {
    "I'm a video editor based in Portugal. For about ten years I've worked with brands, agencies, and creative teams around the world.":
        "Sou editora de vídeo, em Portugal. Há cerca de dez anos que trabalho com marcas, agências e equipas criativas em todo o mundo.",
    "Most of what I do is commercials, brand films, YouTube, social content, and corporate video. I care most about the story underneath the footage, since that's usually what decides whether an edit lands.":
        "A maior parte do que faço são anúncios, filmes de marca, YouTube, conteúdo para redes sociais e vídeo corporativo. O que me importa mais é a história por baixo das imagens, porque é normalmente isso que decide se uma montagem resulta.",
    "I work in English, Portuguese, and Spanish, which helps when a brand is making the same thing for more than one market.":
        "Trabalho em inglês, português e espanhol, o que ajuda quando uma marca faz a mesma peça para mais do que um mercado.",
    "When there's overflow work, a tight deadline, or a client asking for more video than your team can make on its own, I work behind the scenes as your post-production partner. Everything ships under your brand, not mine.":
        "Quando há trabalho a mais, um prazo apertado ou um cliente a pedir mais vídeo do que a vossa equipa consegue produzir, trabalho nos bastidores como vosso parceiro de pós-produção. Tudo sai com a vossa marca, não com a minha.",
    "NDA-friendly, always": "Sempre sob NDA",
    "Your client relationships stay yours — I never appear in the credits or the conversation.":
        "As vossas relações com os clientes continuam vossas — nunca apareço nos créditos nem na conversa.",
    "Built for turnaround": "Feito para prazos",
    "Send me a brief and raw footage; get broadcast-ready edits back on your timeline, not mine.":
        "Enviam-me o briefing e o material bruto; recebem montagens prontas a emitir no vosso prazo, não no meu.",
    "Scales with your pipeline": "Acompanha o vosso ritmo",
    "One-off overflow, or an ongoing monthly retainer. Either works.":
        "Um projeto pontual ou um avençado mensal. Qualquer um funciona.",
    "Tell me a little about your project and I'll get back to you personally.":
        "Conte-me um pouco sobre o seu projeto e respondo-lhe pessoalmente.",
    "This opens your email app with everything filled in — just press send.":
        "Isto abre a sua aplicação de email com tudo preenchido — é só enviar.",
    "Notes on editing, post-production, and working with video.":
        "Notas sobre montagem, pós-produção e trabalhar com vídeo.",
}

META = {
    "title": "Catarina Fidalgo — Editora de Vídeo e Pós-Produção",
    "description": "Catarina Fidalgo — editora de vídeo e pós-produção, em Portugal. "
                   "Anúncios, filmes de marca, YouTube, conteúdo social e vídeo corporativo, "
                   "em inglês, português e espanhol.",
}

# Video types (the purple tags and the contact form list)
TYPES = {
    "Brand Film": "Filme de Marca",
    "Corporate / Promo Video": "Vídeo Corporativo / Promocional",
    "Corporate Training Video": "Vídeo de Formação",
    "E-Learning / Video Course": "E-Learning / Curso em Vídeo",
    "Event Recap Video": "Vídeo de Resumo de Evento",
    "Event Video": "Vídeo de Evento",
    "Explainer Video": "Vídeo Explicativo",
    "Highlight Reel": "Reel de Destaques",
    "Social Media Video": "Vídeo para Redes Sociais",
    "TV Commercial / Ad": "Anúncio de Televisão",
    "Testimonial Video": "Vídeo de Testemunho",
    "YouTube Video": "Vídeo para YouTube",
}

TASKS = {
    "Animated Elements": "Elementos animados",
    "Animated Text and Elements": "Texto e elementos animados",
    "Audio Mixing": "Mistura de áudio",
    "Color Correction": "Correção de cor",
    "Color Grading": "Tratamento de cor",
    "Color Grading and Correction": "Tratamento e correção de cor",
    "Editing": "Edição",
    "Keying": "Recorte de croma",
    "Licensed Music": "Música licenciada",
    "Licensed Stock Footage": "Imagens de arquivo licenciadas",
    "Licensed Stock Footage and Music": "Imagens de arquivo e música licenciadas",
    "Motion Graphics": "Motion graphics",
    "Motion Graphics and Animation": "Motion graphics e animação",
    "Motion Text": "Texto animado",
    "Motion Titles": "Títulos animados",
    "Motion Titling": "Titulagem animada",
    "Sound Design": "Sound design",
    "Sound Mixing": "Mistura de som",
    "Sound Mixing and Sound Design": "Mistura de som e sound design",
    "Subtitling": "Legendagem",
    "Titling": "Titulagem",
}

INDUSTRIES = {
    "Academic Education": "Ensino superior", "Advertising Agency": "Agência de publicidade",
    "Aerospace & Avionics Engineering": "Engenharia aeronáutica", "Air-Condition Services": "Climatização",
    "Artificial Intelligence (AI)": "Inteligência artificial (IA)", "Auto Dealership": "Concessionário automóvel",
    "Banking": "Banca", "Beauty & Cosmetics": "Beleza e cosmética",
    "Books & Literature": "Livros e literatura", "Books Publishing": "Edição de livros",
    "Community & Clubs": "Comunidade e clubes", "Construction": "Construção",
    "Crypto & Blockchain": "Cripto e blockchain", "Dancing": "Dança",
    "Data Analytics": "Análise de dados", "Design Agency": "Agência de design",
    "Developmental Therapy": "Terapia do desenvolvimento", "E-book Publishing": "Edição de e-books",
    "Early Age Education": "Educação de infância", "Education": "Educação",
    "Education Organization": "Organização educativa", "Engineering": "Engenharia",
    "Event Videos": "Vídeos de evento", "Events & Celebrations": "Eventos e celebrações",
    "Fashion & Apparel": "Moda e vestuário", "Fast Food": "Fast food",
    "Financial & Business Services": "Serviços financeiros e empresariais", "Fine Arts": "Belas-artes",
    "Fintech": "Fintech", "Food & Beverage": "Alimentação e bebidas",
    "Food & Beverage Blog/Channel": "Canal de alimentação e bebidas", "Food Manufacturing": "Indústria alimentar",
    "Forestry": "Floresta", "HR & Recruiting Services": "Recursos humanos e recrutamento",
    "Health Care & Medical Center": "Saúde e centros médicos", "Home Supplies": "Materiais para o lar",
    "Hospitality": "Hotelaria", "Industrial Manufacturing & Storage": "Indústria e armazenamento",
    "Law": "Direito", "Logistics": "Logística",
    "Maintenance & Installation": "Manutenção e instalação", "Manufacturing & Storage": "Produção e armazenamento",
    "Marketing & Advertising": "Marketing e publicidade", "Marketing Agency": "Agência de marketing",
    "Media & Entertainment": "Media e entretenimento", "Medical & Health Education": "Educação médica e da saúde",
    "Medical & Pharmaceutical": "Medicina e farmacêutica", "Mental Health Care": "Saúde mental",
    "Musical Event": "Evento musical", "Nature Conservation & Biodiversity": "Conservação da natureza",
    "Night Clubs": "Discotecas", "Non-Profit": "Sem fins lucrativos",
    "Nutrition": "Nutrição", "Personal Consulting": "Consultoria pessoal",
    "Plumbing": "Canalização", "Products Supplying/Manufacturing": "Produção e fornecimento",
    "Professional Services & Consulting": "Serviços profissionais e consultoria", "Psychology": "Psicologia",
    "Renewable Energy": "Energias renováveis", "Research Foundation/Research": "Investigação",
    "Residential Real Estate": "Imobiliário residencial", "Science": "Ciência",
    "Services": "Serviços", "Software Development": "Desenvolvimento de software",
    "Solar Energy": "Energia solar", "Spa": "Spa",
    "Teaching & Lecturing": "Ensino e formação", "Technology": "Tecnologia",
    "Technology Consulting": "Consultoria tecnológica", "Travel & Tourism": "Viagens e turismo",
    "Travel & Tourism Agency": "Agência de viagens", "Virtual Reality": "Realidade virtual",
    "Wind Energy": "Energia eólica", "Youth Education": "Educação de jovens",
}

# Project descriptions, keyed by brand
DESCS = {
 "Coca-Cola": "Vídeo corporativo para uso interno, com materiais da Coca-Cola Europacific e imagens de arquivo licenciadas.",
 "UNESCO": "Vídeo corporativo para uso interno, com voz-off e imagens de arquivo licenciadas, para a UNESCO e o seu programa O3 (Our Rights, Our Lives, Our Future).",
 "FAT Brands Inc.": "Entrevistas com master franchisados da FAT Brands, montadas num reel narrativo de destaques. A FAT Brands detém a Fatburger, a Johnny Rockets, a Round Table Pizza e a Marble Slab Creamery, entre outras.",
 "Moroccanoil": "Vídeos curtos para as redes sociais da Moroccanoil.",
 "Espanita Tequila": "Campanha para a Espanita Tequila, uma tequila feita em colaboração com o Pitbull. Montei também uma versão curta para redes sociais.",
 "Volkswagen": "Anúncio de televisão de 30 segundos para o concessionário Hawk Volkswagen de Monroeville.",
 "99Bitcoins": "Vídeos educativos de formato longo para o 99Bitcoins, um canal de YouTube que explica cripto a principiantes — 710 mil subscritores e mais de 50 milhões de visualizações. Mostram-se aqui duas amostras.",
 "Binance": "Vídeo corporativo com imagens de arquivo licenciadas, voz-off e motion graphics para a Binance Charity e o projeto \\'Crypto Against Covid\\'.",
 "Fairmont Hotels": "3 reels curtos para as redes sociais dos hotéis Fairmont.",
 "Danone": "Série de vídeos para redes sociais, em horizontal e vertical, com texto e elementos animados para o Danone Nutricia Campus — uma plataforma educativa sem fins lucrativos para profissionais de saúde.",
 "The Washington Ballet": "Campanha completa de promoção da temporada 22/23 do Washington Ballet, montada ao ritmo de música clássica.",
 "Capgemini": "Vídeo promocional corporativo de 60\\' com imagens de arquivo e elementos animados para a Capgemini, o grupo de consultoria e serviços tecnológicos.",
 "KAUST": "Entrevistas e um filme de marca sobre o programa SRSI da King Abdullah University of Science and Technology (KAUST).",
 "Litify": "Vídeo promocional corporativo e vídeo de evento com motion graphics para a Litify, uma plataforma jurídica criada por advogados para advogados.",
 "SmallPond": "Campanha social completa para a SmallPond, uma empresa de viagens de grupo — dois anúncios principais (um horizontal de 30 segundos e uma versão quadrada de 60) e cinco reels verticais para Instagram.",
 "Thetaray": "Vídeo sem som, com elementos animados, para um ecrã de stand em feira. A ThetaRay desenvolve software de monitorização de transações com IA para bancos e prestadores de pagamentos.",
 "Peter Jackson": "Vídeos promocionais para as redes sociais da Peter Jackson, uma marca de moda masculina.",
 "SimilarWeb": "Vídeo explicativo com texto e elementos animados para a SimilarWeb, uma empresa de análise de dados web.",
 "The Snow Lodge": "Vídeo de resumo de evento para redes sociais do The Snow Lodge, um conceito de entretenimento de inverno.",
 "APEL": "Vídeos promocionais de evento com elementos animados, voz-off e imagens de arquivo licenciadas para a APEL (Associação Portuguesa de Editores e Livreiros).",
 "Bianco+Ye": "Vídeo de identidade de marca com títulos animados para a Bianco+Ye, uma agência criativa full-service cofundada por Yerong Xu e Simone Bianconcini.",
 "Apex Imaging Services": "Vídeos corporativos trimestrais para uso interno, com motion graphics, para a Apex Imaging Services, sobre resultados e planos para o ano seguinte.",
 "The AI School": "Curso em vídeo de 2 horas com 9 lições, com motion graphics, para a The AI School. Mostram-se aqui apenas duas lições de amostra, não o curso completo.",
 "You've Been Desserted": "Vídeos de YouTube com elementos animados para \\'You\\'ve Been Desserted\\', um programa multicâmara apresentado pelo pasteleiro e cake designer Joshua John Russell. Montei cada episódio em estreita colaboração com os produtores, mantendo o timing cómico apertado. Mostram-se aqui três episódios de amostra; foram montados muitos mais.",
 "Perfect Jeans": "Anúncio para a Perfect Jeans, uma marca dinamarquesa de calças de ganga. Para site, redes sociais e televisão (TV2). O público-alvo são mulheres entre os 35 e os 65 anos e famílias.",
 "ThinkShop By The Garden": "Vídeo explicativo com entrevista e imagens de apoio para a The Garden, a promover o programa de desenvolvimento profissional ThinkShop.",
 "Southern Pipe & Supply": "Vídeo de testemunho multicâmara de 3 minutos com colaboradores da Southern Pipe. A Southern Pipe & Supply é fornecedora de canalização, AVAC e material industrial no sudeste dos Estados Unidos.",
 "Savage Training Group": "18 vídeos de formação para a Savage Training Group, uma organização de formação de forças de segurança. Mostram-se aqui apenas três lições de amostra, não as 18.",
 "Five Oaks": "Vídeos educativos, vídeos de evento e anúncios para a Five Oaks, um centro de investigação e educação dedicado à conservação de zonas húmidas e às populações de aves aquáticas.",
 "FEAM Aero": "Vídeo corporativo sem som, com elementos animados, para um stand de feira, para a FEAM Aero — manutenção de aeronaves e serviços de linha.",
}
